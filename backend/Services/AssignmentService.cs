...
public sealed class AssignmentService(ApplicationDbContext dbContext)
{
    ...
    public async Task<AssignmentOperationResult> CreateAsync(AssignmentCreateDto request, string currentUserId, CancellationToken ct)
    {
        ...
        if (!asset.Status.Equals(AssetLifecycleRules.Available, StringComparison.OrdinalIgnoreCase))
            return Conflict("Yalnızca Boşta durumundaki cihazlar zimmetlenebilir.");
        if (await dbContext.Assignments.AnyAsync(x => x.AssetId == asset.Id && x.ReturnedAt == null, ct))
            return Conflict("Bu cihazın zaten aktif bir zimmeti var.");

        var assignment = new Assignment
        {
            Id = Guid.NewGuid().ToString("N"), AssetId = asset.Id, EmployeeId = employee.Id,
            AssignedAt = request.AssignedAt!.Value, AssignedByUserId = currentUserId,
            Notes = Clean(request.Notes), CreatedAt = DateTimeOffset.UtcNow,
            Asset = asset, Employee = employee
        };
        var previousStatus = asset.Status;
        asset.Status = AssetLifecycleRules.Assigned;
        dbContext.Assignments.Add(assignment);
        dbContext.AssetMovements.Add(AssetMovementFactory.Create(
            asset.Id, AssetMovementType.Assigned, assignment.AssignedAt,
            previousStatus, asset.Status, currentUserId,
            $"Cihaz {employee.FullName} adlı çalışana zimmetlendi.",
            relatedEntityType: nameof(Assignment), relatedEntityId: assignment.Id));
    ...
    public async Task<AssignmentOperationResult> ReturnAsync(string id, AssignmentReturnDto request, string currentUserId, CancellationToken ct)
    {
        ...
        assignment.Asset.Status = AssetLifecycleRules.Available;
        dbContext.AssetMovements.Add(AssetMovementFactory.Create(
            assignment.AssetId, AssetMovementType.Returned, assignment.ReturnedAt.Value,
            previousStatus, assignment.Asset.Status, currentUserId,
            $"{assignment.Employee.FullName} adlı çalışanın cihaz iadesi alındı.",
            relatedEntityType: nameof(Assignment), relatedEntityId: assignment.Id));
    ...
    private static IQueryable<Assignment> ApplyFilters(IQueryable<Assignment> query, string? search, string? department)
    {
        if (!string.IsNullOrWhiteSpace(search))
        {
            var normalizedSearch = search.Trim();
            query = query.Where(x => x.Asset.AssetCode.Contains(normalizedSearch) ||
                x.Employee.FullName.Contains(normalizedSearch) || x.Asset.Brand.Contains(normalizedSearch) ||
                x.Asset.Model.Contains(normalizedSearch));
        }
        if (!string.IsNullOrWhiteSpace(department))
        {
            var normalizedDepartment = department.Trim();
            query = query.Where(x => x.Employee.Department == normalizedDepartment);
        }
        return query;
    }

    private static Task<List<AssignmentDto>> Read(IQueryable<Assignment> query, CancellationToken ct) =>
        query.OrderByDescending(x => x.AssignedAt).Select(ToDto()).ToListAsync(ct);
